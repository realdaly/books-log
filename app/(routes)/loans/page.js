...
export default function LoansPage() {
    ...
    const COLUMNS = [
        ...
        { id: 'party_name', label: 'الجهة المستعيرة', accessor: r => r.party_name || "-" },
        ...
        >
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <h1 className="text-xl md:text-3xl font-bold text-primary">سجل الاستعارة</h1>
                    ...
                    <table className="w-full text-right text-sm border-collapse border-b border-border">
                        <thead className="bg-primary text-primary-foreground font-bold sticky top-0 z-10 shadow-md">
                            <tr>
                                ...
                                <th onClick={(e) => handleColumnClick(4, e)} className={`p-4 border-l border-primary-foreground/10 w-1/2 text-right cursor-pointer transition-colors ${selectedCols.has(4) ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-900 dark:text-blue-100' : ''}`}>الجهة المستعيرة</th>
                        ...
                        <tbody className="divide-y divide-border">
                            ...
                            {!loading && transactions.map((t, idx) => (
                                <tr key={t.id} className={`odd:bg-muted/30 even:bg-card hover:bg-primary/5 transition-colors ${selectedIds.includes(t.id) ? 'bg-primary/10' : ''}`}>
                                    ...
                                    <td className={`p-4 text-center text-muted-foreground border-l border-border/50 tracking-tighter ${selectedCols.has(2) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
            ...
            <Modal
                ...
                title={editId ? "تعديل استعارة" : (isMultiMode ? "إضافة استعارات متعددة" : "إضافة استعارة جديدة")}
                    ...
                    {!isMultiMode ? (
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center bg-muted/30 p-3 rounded-lg border border-dashed border-border">
                                <span className="text-sm font-bold text-primary">اختر الكتب المراد استعارتها ({selectedMultiBooks.length}):</span>
