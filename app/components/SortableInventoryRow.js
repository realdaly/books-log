...
export function SortableInventoryRow({ row, index, updateField, successMap, selectedCols, threshold }) {
    ...
    return (
        ...
        >
            ...
            {/* Sent to Inst */}
            <td className={`p-3 text-center text-foreground border-l border-border/50 ${selectedCols.has(5) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
            ...
            {/* Pending Sale */}
            <td className={`p-3 text-center text-foreground border-l border-border/50 ${selectedCols.has(7) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>{row.pending_institution || '-'}</td>

            {/* Inst Stats */}
            <td className={`p-3 text-center text-foreground border-l border-border/50 ${selectedCols.has(8) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>{row.sold_institution}</td>
            <td className={`p-3 text-center text-foreground border-l border-border/50 ${selectedCols.has(9) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>{row.gifted_institution}</td>
            <td className={`p-3 text-center text-foreground border-l border-border/50 ${selectedCols.has(10) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>{row.loaned_institution}</td>

            {/* Manual Loss */}
            <td className={`p-3 text-center text-foreground border-l border-border/50 ${selectedCols.has(11) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                {row.loss_manual}
                {row.loss_institution > 0 && <div className="text-[10px] text-red-500 font-bold mt-0.5">+{row.loss_institution}</div>}
            </td>

            {/* New Stores (Makhazen) */}
            <td className={`p-3 text-center text-foreground border-l border-l-border/50 border-r border-r-primary-foreground/10 ${selectedCols.has(12) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
            ...
            <td className={`p-3 text-center font-bold text-foreground border-l border-l-border/50 border-r border-r-primary-foreground/10 ${selectedCols.has(13) ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
            ...
            <td className={`p-3 text-center font-black text-lg text-primary transition-colors border-l border-border/50 ${selectedCols.has(14) ? 'bg-blue-100 dark:bg-blue-900/30' : 'bg-primary/5 group-hover:bg-primary/20'}`}>
                {row.remaining_total}
            </td>
        </tr>
    );
